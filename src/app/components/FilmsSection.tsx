...
export function FilmsSection() {
  ...
  return (
    <section id="films" className="min-h-screen py-24 px-6 bg-[#333d29]">
      <div className="max-w-7xl mx-auto">
        ...
        <Masonry columnsCount={3} gutter="1.5rem">
          {films.map((film, index) => (
            ...
            >
              <div className="relative overflow-hidden rounded-lg bg-[#414833]">
                {/* Thumbnail */}
                <div className="relative aspect-[4/3] overflow-hidden">
                  ...
                  />
                  {/* Overlay */}
                  <div className="absolute inset-0 bg-gradient-to-t from-[#582f0e]/90 via-[#582f0e]/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                    <div className="w-16 h-16 rounded-full bg-[#7f4f24] flex items-center justify-center transform scale-90 group-hover:scale-100 transition-transform duration-300">
                      <Play className="w-8 h-8 text-[#c2c5aa] ml-1" fill="currentColor" />
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4">
                  <p
                    className="text-xs uppercase tracking-wider text-[#a4ac86] mb-1"
                    style={{ fontFamily: 'SF Pro Text, -apple-system, sans-serif' }}
                  >
                    {film.category}
                  </p>
                  <h3
                    className="text-xl text-[#c2c5aa]"
      ...
      <AnimatePresence>
        {selectedFilm && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-[#582f0e]/95 backdrop-blur-sm"
          ...
          >
            <motion.div
              ...
              className="relative w-full max-w-5xl bg-[#414833] rounded-lg overflow-hidden shadow-2xl"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Close Button */}
              <button
                onClick={() => setSelectedFilm(null)}
                className="absolute top-4 right-4 z-10 w-10 h-10 rounded-full bg-[#582f0e] text-[#b6ad90] hover:bg-[#7f4f24] transition-colors flex items-center justify-center"
              >
                <X className="w-6 h-6" />
              </button>
              ...
              <div className="p-6">
                <p
                  className="text-sm uppercase tracking-wider text-[#a4ac86] mb-2"
                  style={{ fontFamily: 'SF Pro Text, -apple-system, sans-serif' }}
                >
                  {selectedFilm.category}
                </p>
                <h3
                  className="text-3xl text-[#b6ad90]"
